...
public class QuizFlowTest {
    ...
    public void setUp() {
        ...
        student = new Student("testStudent", "password", "Test Student");
    ...
    public void mainTest() {
        ...
        QuizSession quizSession = quizService.startQuizSession(student, topic, 2);
        assertNotNull(quizSession);
        assertEquals(0, student.getTakenQuizzes().size());

        for(Question q : quizSession.getQuestions()) {
            q.setCurrentAnswer(String.valueOf(2));
        }

        quizService.finishQuizSession(quizSession);

        assertNotEquals(100.0, quizSession.getReportSummary().getScorePercentage(), 0.01);
        assertEquals(1, student.getTakenQuizzes().size());
        assertEquals(quizSession.getSessionId(), student.getTakenQuizzes().getFirst().getSessionId());
    }
}
